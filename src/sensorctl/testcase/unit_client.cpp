...
TESTCASE(sensor_api_provider, provider_base_p_1)
{
	bool ret;
	int handle;

	handle = sensord_external_connect("TEST", NULL, NULL);
	ASSERT_GE(handle, 0);

	for (int k = 0; k < 1000; ++k) {
		float data[3] = { 1, 2, 3 };
		ret = sensord_external_post(handle, 1, data, 3);
		ASSERT_TRUE(ret);
		sleep(1);
	}

	ret = sensord_external_disconnect(handle);
	ASSERT_TRUE(ret);
